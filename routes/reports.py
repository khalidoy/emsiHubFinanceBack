...
def calculate_monthly_data(month_num, year, school_year_period_id):
    ...
    end_date = datetime(year + 1, 1, 1, tzinfo=timezone.utc) if month_num == 12 \
        else datetime(year, month_num + 1, 1, tzinfo=timezone.utc)

    # Sum expenses
    expenses = Depence.objects(Q(date__gte=start_date) & Q(date__lt=end_date))
    total_expenses = sum(dep.amount for dep in expenses)

    net_profit = total_monthly_agreed_payments - total_expenses
...
def normal_profit_report():
    try:
        ...
        report_data = []
        # Months: September-December of start year
        for month_num in range(9, 13):
            report_data.append(calculate_monthly_data(month_num, start_year, school_year_period_id))

        # Months: January-June of end year
        for month_num in range(1, 7):
            report_data.append(calculate_monthly_data(month_num, end_year, school_year_period_id))
...
def unknown_agreed_payments():
    """Lists students with all monthly agreed payments = 0"""
    try:
        school_year_period_id = request.args.get('schoolyear_id')
        if not school_year_period_id:
            return jsonify({"status": "error", "message": "School Year Period ID is required"}), 400
        ...
        return jsonify({
            "status": "success",
            "count": len(json.loads(unknown_students)),
            "students": json.loads(unknown_students)
        }), 200
